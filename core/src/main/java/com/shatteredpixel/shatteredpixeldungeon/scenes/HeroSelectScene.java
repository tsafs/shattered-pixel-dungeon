...
public class HeroSelectScene extends PixelScene {
    ...
    private class GameOptions extends Component {
        ...
        protected void createChildren() {
            ...
            StyledButton dailyButton = new StyledButton(Chrome.Type.BLANK,
                    Messages.get(HeroSelectScene.class, "daily"), 6) {
                    ...
                    for (GamesInProgress.Info game : GamesInProgress.checkAll()) {
                        if (game.daily) {
                            ShatteredPixelDungeon.scene().addToFront(
                                    new WndMessage(Messages.get(HeroSelectScene.class, "daily_existing")));
                            return;
                        }
                    }
                ...
                protected void onClick() {
                    ShatteredPixelDungeon.scene().addToFront(new WndChallenges(SPDSettings.challenges(), true) {
                        public void onBackPressed() {
                            super.onBackPressed();
                            icon(Icons
                                    .get(SPDSettings.challenges() > 0 ? Icons.CHALLENGE_ON : Icons.CHALLENGE_OFF));
                            updateOptionsColor();
                        }
                    });
                }
            ...
            CheckBox chkIsHardcoreMode = new CheckBox(Messages.get(WndGameInProgress.class, "hardcore")) {
                ...
                protected void onClick() {
                    super.onClick();
                    SPDSettings.isHardcoreMode(checked());
                }
            };
            chkIsHardcoreMode.checked(Dungeon.isHardcoreMode);
            chkIsHardcoreMode.leftJustify = true;
            add(chkIsHardcoreMode);
            buttons.add(chkIsHardcoreMode);
        ...
        protected void layout() {
            ...
            for (StyledButton btn : buttons) {
                btn.setRect(x + bg.marginLeft(), top, btn.reqWidth(), 16);
                top = (int) btn.bottom();
                if (i < spacers.size()) {
